 ...
 * 类 名 称：TaskTaskHandlerBLL(TaskTaskHandlerBLL)
 * 功能描述：
 * 
 * ------------创建信息------------------
 * 作    者：jjf001
...
{
    /// <summary>
    /// BusinessHandler TaskTaskHandlerBLL
    /// </summary>
    public class TaskTaskHandlerBLL:BaseBLL
    {
		//申明DAL接口
		private TaskTaskHandlerDAL Provider = DALFactory.Instance().TaskTaskHandlerDAL;
		
		#region constructor
        /// <summary>
        /// TaskTaskHandlerBLL
        /// </summary>
        public TaskTaskHandlerBLL()
        {
        }
		#endregion
        ...
        public SystemMessage Add(TaskTaskHandler entity)
        {
        ...
        /// <param name="entity"></param>
        /// <returns>return the handler result</returns>
        public SystemMessage Edit(TaskTaskHandler entity)
        { 
            //TaskTaskHandler originEntity = Provider.GetData(entity.Name);
            ...
            //    return HandlerMessage;
            //}
            TaskTaskHandler originEntity = Provider.GetData(entity.ID);
        ...
        ///<param name="id"></param>
        /// <returns></returns>
        public TaskTaskHandler GetData(int id)
        {           
			try
			{
        ...
        /// </summary>
        /// <returns></returns>
        public List<TaskTaskHandler> GetList()
        {
			try
			{
        ...
        /// </summary>
        /// <returns></returns>
        public List<TaskTaskHandler> GetListByTaskID(int id)
        {
            try
            {
                return Provider.GetDatasByProperty("TaskID", id);
            }
            catch (Exception ex)
            {
                // 将错误日志写入数据库
        ...
        /// <returns></returns>
        public List<TaskTaskHandler> GetList(List<SimpleExpression> exps)
        {
			try
			{
        ...
        /// <returns></returns>
        public List<TaskTaskHandler> GetList(PagerData pager)
        {
			try
			{
		...
		/// <param name="exps">查询条件</param>
        /// <returns></returns>
        public List<TaskTaskHandler> GetList(PagerData pager,List<SimpleExpression> exps)
        {
			try
			{
        ...
        /// <returns></returns>
        public List<TaskTaskHandler> GetList(PagerData pager,OrderItem order)
        {
			try
			{
        ...
        /// <returns></returns>
        public List<TaskTaskHandler> GetList(PagerData pager,OrderItemCollection orders)
        {
			try
			{
		...
		/// <param name="exps">查询条件</param>
        /// <returns></returns>
        public List<TaskTaskHandler> GetList(PagerData pager,OrderItemCollection orders,List<SimpleExpression> exps)
        {
			try
			{
        ...
        /// <returns></returns>
        public SystemMessage Remove(string property, object value)
        {
            HandlerMessage.Code = "01";
            HandlerMessage.Text = "删除失败";
            HandlerMessage.Succeed = false;
            List<SimpleExpression> exps = new List<SimpleExpression>();
            exps.Add(Expression.Eq(property, value));
