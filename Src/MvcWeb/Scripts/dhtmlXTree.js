...
function dtmlXMLLoaderObject(funcObject,dhtmlObject){
 ...
 if(z)return z;
 //alert("Incorrect XML");  //防止页面切换过快的时候抛出此提示
...
function dhtmlXTreeObject(htmlObject,width,height,rootId){
 ...
 // Modified : default directory for images
 ...
 this.imageArray=new Array("user3.gif","folderOpen.gif","user3.gif");//user3.gif为无权限状态
...
// Modified : transfer parameters pAAA, pBBB, pCCC to the function to new a tree item object
function dhtmlXTreeItemObject(itemId,itemText,parentObject,treeObject,pNodeType,pData1,pData2,pData3,pData4,pData5,pData6,actionHandler,mode){
 this.htmlNode="";
 this.acolor="";
 this.scolor="";
 this.tr=0;
 ...
 // Modified : evaluate the parameters relavant to the properties in the xml file
 ...
 dhtmlXTreeObject.prototype._drawNewTr=function(htmlObject)
{
 ...
 // Modified : add parameters pNodeType, pData1, pData2, pData3, pData4, pData5, pData6 to the method inherited from tree object
 dhtmlXTreeObject.prototype._attachChildNode=function(parentObject,itemId,itemText,itemActionHandler,image1,image2,image3,pNodeType,pData1, pData2,pData3,pData4,pData5,pData6,optionStr,childs,beforeNode){
 ...
 if(((parentObject.XMLload==0)&&(this.XMLsource))&&(!this.XMLloadingWarning))
{
 ...
 // Modified : create a tree item object by new
 Nodes[Count]=new dhtmlXTreeItemObject(itemId,itemText,parentObject,this,pNodeType,pData1, pData2,pData3,pData4,pData5,pData6,itemActionHandler,1);
 
 if(image1)Nodes[Count].images[0]=image1;
 if(image2)Nodes[Count].images[1]=image2;
 if(image3)Nodes[Count].images[2]=image3;
 // Modified : envaluate the properties in tree item object
 if(pNodeType)Nodes[Count].nodetype=pNodeType;
 if(pData1)Nodes[Count].data1=pData1;
 if(pData2)Nodes[Count].data1=pData2;
 if(pData3)Nodes[Count].data1=pData3;
 if(pData4)Nodes[Count].data1=pData4;
 if(pData5)Nodes[Count].data1=pData5;
 if(pData6)Nodes[Count].data1=pData6
 // End of Modified
 ...
 if(parentObject.childsCount>=2)
{
 ...
 // Modified : method to insert a new item by the given parms
 dhtmlXTreeObject.prototype.insertNewItem=function(parentId,itemId,itemText,itemActionHandler,image1,image2,image3,pNodeType,pData1, pData2,pData3,pData4,pData5,pData6,optionStr,childs){
 var parentObject=this._globalIdStorageFind(parentId);
 if(!parentObject)return(-1);
 // Modified : method to attach a new item by the given parms
 return this._attachChildNode(parentObject,itemId,itemText,itemActionHandler,image1,image2,image3,pNodeType,pData1, pData2,pData3,pData4,pData5,pData6,optionStr,childs);
};
 ...
 if((node.childNodes[i].nodeType==1)&&(node.childNodes[i].tagName == "item"))
{
 ...
 // Modified : retrieve the properties from xml file , aa,bb,cc is temporary variables
 var nodetype=node.childNodes[i].getAttribute("nodetype");
 var data1=node.childNodes[i].getAttribute("data1");
 var data2=node.childNodes[i].getAttribute("data2");
 var data3=node.childNodes[i].getAttribute("data3");
 var data4=node.childNodes[i].getAttribute("data4");
 var data5=node.childNodes[i].getAttribute("data5");
 var data6=node.childNodes[i].getAttribute("data6");  
 // End of Modified
 ...
 // Modified : call this method to insert a new item to the tree, be sure to transfer the parms(aa,bb,cc) if you need them
 dhtmlObject.insertNewItem(parentId,cId,name,0,im0,im1,im2,nodetype,data1,data2,data3,data4,data5,data6,zST,chd);
...
dhtmlXTreeObject.prototype._correctCheckStates=function(dhtmlObject){
 ...
 td12.appendChild(img);img.style.padding=0;img.style.margin=0;img.style.marginRight='3px';
// if(this.timgen)
...
//{img.style.width="0px";img.style.height="0px";}
 ...
 // Modified : hidden span for other usage
 itemObject.span1=document.createElement('span');
 itemObject.span1.style.display="none";
 itemObject.span1.appendChild(document.createTextNode(itemObject.nodetype));
 td2.appendChild(itemObject.span1); 

 itemObject.span2=document.createElement('span');
 itemObject.span2.style.display="none";
 itemObject.span2.appendChild(document.createTextNode(itemObject.data1));
 td2.appendChild(itemObject.span2);

 itemObject.span3=document.createElement('span');
 itemObject.span3.style.display="none";
 itemObject.span3.appendChild(document.createTextNode(itemObject.data2));
 td2.appendChild(itemObject.span3);
 
 itemObject.span4=document.createElement('span');
 itemObject.span4.style.display="none";
 itemObject.span4.appendChild(document.createTextNode(itemObject.data3));
 td2.appendChild(itemObject.span4);
 
 itemObject.span5=document.createElement('span');
 itemObject.span5.style.display="none";
 itemObject.span5.appendChild(document.createTextNode(itemObject.data4));
 td2.appendChild(itemObject.span5);
 
 itemObject.span6=document.createElement('span');
 itemObject.span6.style.display="none";
 itemObject.span6.appendChild(document.createTextNode(itemObject.data5));
 td2.appendChild(itemObject.span6); 
 
 itemObject.span7=document.createElement('span');
 itemObject.span7.style.display="none";
 itemObject.span7.appendChild(document.createTextNode(itemObject.data6));
 td2.appendChild(itemObject.span7); 
 ...
 dhtmlXTreeObject.prototype.getItemText=function(itemId)
{
 ...
 dhtmlXTreeObject.prototype.getNodeTypeText=function(itemId)
{
 var temp=this._globalIdStorageFind(itemId);
 if(!temp)return 0;
 return(temp.htmlNode.childNodes[0].childNodes[0].childNodes[3].childNodes[1].innerHTML);
 //return(temp.htmlNode.childNodes[0].childNodes[0].childNodes[3].childNodes[0].innerHTML);
};

 dhtmlXTreeObject.prototype.getParmText=function(itemId,parmName)
{
 var parmId;
 switch(parmName){
	case 'nodetype': parmId = 1;break;
	case 'data1': parmId = 2;break;
	case 'data2': parmId = 3;break;
	case 'data3': parmId = 4;break;
	case 'data4': parmId = 5;break;
	case 'data5': parmId = 6;break;	
	case 'data6': parmId = 7;break;		
	default:break;
 }
 var temp=this._globalIdStorageFind(itemId);
 if(!temp)return 0;
 return(temp.htmlNode.childNodes[0].childNodes[0].childNodes[3].childNodes[parmId].innerHTML);
 //return(temp.htmlNode.childNodes[0].childNodes[0].childNodes[3].innerHTML);
};
...
dhtmlXTreeObject.prototype._moveNode=function(itemObject,targetObject){
    if(this.dragFunc){
        if(!this.dragFunc(itemObject.id,targetObject.id)) return false;
    }
 ...
 for(var i=0;i<sNode.childsCount;i++)
{
 ...
 dhtmlXTreeObject.prototype._getAllChecked=function(htmlNode,list,mode){
  ...
  // Modified : get all checked with nodeType=0
 dhtmlXTreeObject.prototype.getAllCheckedByNodetype=function(nodeType){
 return this._getAllCheckedByNodetype("","",1,nodeType);
}

 dhtmlXTreeObject.prototype.getAllCheckedBranchesByNodetype=function(nodeType){
 return this._getAllCheckedByNodetype("","",0,nodeType);
}

 dhtmlXTreeObject.prototype._getAllCheckedByNodetype=function(htmlNode,list,mode,nodeType){
     if(!htmlNode)htmlNode=this.htmlNode;
     ...
     if( ((mode)&&(htmlNode.checkstate==1)) || ((!mode)&&(htmlNode.checkstate>0)) )
     {
        if(list)
        {
            // predicate of the node type
             var temp=this._globalIdStorageFind(htmlNode.id);
             if(!temp)return;
             var type = temp.htmlNode.childNodes[0].childNodes[0].childNodes[3].childNodes[1].innerHTML;
             if( type == nodeType) list+=","+htmlNode.id;             
        }
        else
        {
             var temp=this._globalIdStorageFind(htmlNode.id);
     ...
     for(var i=0;i<j;i++)
        {
         list=this._getAllCheckedByNodetype(htmlNode.childNodes[i],list,mode,nodeType);
        };
     if(list)return list;else return "";
};
 // End of Modified 
 ...
 for(var i=0;i<itemObject.childsCount;i++)
{
 ...
 dhtmlXTreeObject.prototype.insertNewNext=function(parentItemId,itemId,itemName,itemActionHandler,image1,image2,image3,pAAA,pBBB,pCCC,optionStr,childs){
 var sNode=this._globalIdStorageFind(parentItemId);
 if(!sNode)return(0);
 this._attachChildNode(0,itemId,itemName,itemActionHandler,image1,image2,image3,pAAA,pBBB,pCCC,optionStr,childs,sNode);
};
