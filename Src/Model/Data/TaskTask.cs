/******************************************************************
 *
 * 所在模块：Model(数据实体)
 * 类 名 称：TaskTask(TaskTask)
 * 功能描述：
 * 
 * ------------创建信息------------------
 * 作    者：jjf001
 * 日    期：2013-11-07
 * xingzhewujiang1999@gmail.com
 * QQ:970355214
 * ------------编辑修改信息--------------
 * 作    者：
 * 日    期：
 * 内    容：
******************************************************************/
using System;
using System.Collections;
using System.Data;
using System.Xml;

using Hope.Util;
using Hope.ITMS.Enums;

namespace Hope.ITMS.Model
{
    /// <summary>
    /// DataEntity TaskTask
    /// </summary>
    /// <remarks>
    /// 
    /// </remarks>
    [Serializable]
    public class TaskTask : BaseData
    {
        /// <summary>
        /// TaskTask
        /// </summary>
        public TaskTask()
        {
            Initialize();
        }
        
        #region Base Members ...

        /// <summary>
        /// 
        /// </summary>
        public virtual int ID
        {
            set;
            get;
        }

        /// <summary>
        /// 任务名称
        /// </summary>
        public virtual string Name
        {
            set;
            get;
        }

        /// <summary>
        /// 任务类型
        /// </summary>
        public virtual int TypeID
        {
            set;
            get;
        }

        /// <summary>
        /// 任务来源
        /// </summary>
        public virtual string Source
        {
            set;
            get;
        }

        /// <summary>
        /// 进度
        /// </summary>
        public virtual int Progress
        {
            set;
            get;
        }

        /// <summary>
        /// 任务详情
        /// </summary>
        public virtual string Detail
        {
            set;
            get;
        }

        /// <summary>
        /// 提交者
        /// </summary>
        public virtual int Submitor
        {
            set;
            get;
        }

        /// <summary>
        /// 提交时间
        /// </summary>
        public virtual DateTime SubmitTime
        {
            set;
            get;
        }

        /// <summary>
        /// 优先级
        /// </summary>
        public virtual int Priority
        {
            set;
            get;
        }

        /// <summary>
        /// 期限
        /// </summary>
        public virtual DateTime Deadline
        {
            set;
            get;
        }

        /// <summary>
        /// 频率
        /// </summary>
        public virtual int Frequency
        {
            set;
            get;
        }

        /// <summary>
        /// 备注
        /// </summary>
        public virtual string Remark
        {
            set;
            get;
        }

        #endregion
        
        #region Base Methods ...
        
        /// <summary>
        /// 转换为XML节点
        /// </summary>
        /// <param name="parentDoc">父XML文档</param>
        /// <returns>返回XML文档节点</returns>
        public virtual XmlNode ToXmlNode(XmlDocument parentDoc)
        {
            return ToXmlNode(parentDoc, "item");
        }
        
        /// <summary>
        /// 转换为XML节点
        /// </summary>
        /// <param name="parentDoc">父XML文档</param>
        /// <param name="nodeName">节点名</param>
        /// <returns>返回XML文档节点</returns>
        public virtual XmlNode ToXmlNode(XmlDocument parentDoc, string nodeName)
        {
            XmlDocument xml = parentDoc;
            if (xml == null)
            {
                xml = XMLHelper.CreateXmlDoc();
            }
            XmlNode xn = parentDoc.CreateNode("element", nodeName, "");
            
            XmlElement xe;
            xe = CreateElement(xml, Columns.ID.ToString(),ID.ToString());
            xn.AppendChild(xe);
            xe = CreateElement(xml, Columns.Name.ToString(),Name.ToString());
            xn.AppendChild(xe);
            xe = CreateElement(xml, Columns.TypeID.ToString(),TypeID.ToString());
            xn.AppendChild(xe);
            xe = CreateElement(xml, Columns.Source.ToString(),Source.ToString());
            xn.AppendChild(xe);
            xe = CreateElement(xml, Columns.Progress.ToString(),Progress.ToString());
            xn.AppendChild(xe);
            xe = CreateElement(xml, Columns.Detail.ToString(),Detail.ToString());
            xn.AppendChild(xe);
            xe = CreateElement(xml, Columns.Submitor.ToString(),Submitor.ToString());
            xn.AppendChild(xe);
            xe = CreateElement(xml, Columns.SubmitTime.ToString(),SubmitTime.ToString("yyyy-MM-dd HH:mm:ss"));
            xn.AppendChild(xe);
            xe = CreateElement(xml, Columns.Priority.ToString(),Priority.ToString());
            xn.AppendChild(xe);
            xe = CreateElement(xml, Columns.Deadline.ToString(),Deadline.ToString("yyyy-MM-dd HH:mm:ss"));
            xn.AppendChild(xe);
            xe = CreateElement(xml, Columns.Frequency.ToString(),Frequency.ToString());
            xn.AppendChild(xe);
            xe = CreateElement(xml, Columns.Remark.ToString(),Remark.ToString());
            xn.AppendChild(xe);
            
            return xn;
        }
        
        /// <summary>
        /// 设置初始化默认值
        /// </summary>
        private void Initialize()
        {
            ID = 0;
            Name = string.Empty;
            TypeID = 0;
            Source = string.Empty;
            Progress = 0;
            Detail = string.Empty;
            Submitor = 0;
            SubmitTime = DateTime.Now;
            Priority = 0;
            Deadline = DateTime.Now;
            Frequency = 0;
            Remark = string.Empty;
        }
        
        /// <summary>
        /// 
        /// </summary>
        /// <returns></returns>
        public override string ToString()
        {
            return "TaskTask";
        }
        #endregion
        
        #region Order Columns ...

        /// <summary>
        /// 
        /// </summary>
        public enum Columns
        {
            ID,
            Name,
            TypeID,
            Source,
            Progress,
            Detail,
            Submitor,
            SubmitTime,
            Priority,
            Deadline,
            Frequency,
            Remark,
        }

        #endregion
        
    }
}


